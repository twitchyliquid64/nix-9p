...
// For example, the linux 9p kernel client propagates O_DIRECT to TCREATE and TOPEN
...
pub struct Nixfs {
    ...
    pub readwrite: bool,
    pub path_allowlist: Option<Trie<String, ()>>,
}

impl Nixfs {
    fn path_check(&self, path: &PathBuf) -> Result<()> {
        ...
        if let Some(t) = self.path_allowlist.as_ref() {
            match t.get_ancestor_value(&String::from(path.clone().to_string_lossy())) {
                Some(_) => Ok(()),
                None => Err(Error::Io(io_err!(PermissionDenied, "path not permitted"))),
            }
        } else {
            Ok(())
        }
...
impl Filesystem for Nixfs {
    ...
    async fn rsetattr(
        ...
        if !self.readwrite {
            return Err(Error::Io(io_err!(Unsupported, "Read-only filesystem")));
        }
    ...
    async fn rreaddir(&self, fid: &Fid<Self::Fid>, off: u64, count: u32) -> Result<Fcall> {
        ...
        while let Some(entry) = entries.next().await {
            ...
            if !root || self.path_check(&entry.path()).is_ok() {
                dirents.push(dirent);
            }
            i += 1;
        }
    ...
    async fn rrenameat(
        ...
        self.path_check(&oldpath)?;
        self.path_check(&newpath)?;
